...
<task type="auto">
  ...
  <action>
    ...
    4. Add `installSessionStartHookIfNeeded() throws` method to HookInstaller:
       ...
       - Decision logic:
         a) If file MISSING: CREATE it (install bundled hook)
         b) If file EXISTS WITH marker (our hook): UPDATE if version outdated
         c) If file EXISTS WITHOUT marker (user's custom hook): SKIP (don't overwrite)
  ...
  <verify>
    Build succeeds: `xcodebuild -scheme Dispatch -destination 'platform=macOS' build`
    Methods exist: `grep -E "func install(Library|SessionStartHook)IfNeeded" Dispatch/Services/HookInstaller.swift`
    Inspect logs after launching app for installation messages.
  </verify>
  <done>
