...
1. **HotkeyManager.swift changes:**
   ...
   - Add `func registerCaptureHotkeys()` that:
     - Reads capture shortcut settings from SettingsManager
     - Registers region capture hotkey (if configured)
     - Registers window capture hotkey (if configured)
     - Handlers call ScreenshotCaptureService and CaptureCoordinator
