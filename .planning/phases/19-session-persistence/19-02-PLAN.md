...
must_haves:
  truths:
    ...
    - "Stale Claude sessions are detected and claudeSessionId cleared for fresh start"
...
<task type="auto">
  <name>Task 2: Update MultiSessionTerminalView with session loading and picker presentation</name>
  <files>
    Dispatch/Views/Terminal/MultiSessionTerminalView.swift
  </files>
  <action>
Update MultiSessionTerminalView to load persisted sessions on launch and present picker:
...
4. Add stub for PersistedSessionPicker (detailed UI in Task 3):
   ```swift
...
Build succeeds: `xcodebuild -scheme Dispatch -destination 'platform=macOS' build 2>&1 | grep -E "(error:|BUILD SUCCEEDED)"`
  ...
  <done>
MultiSessionTerminalView loads persisted sessions on launch, shows basic picker to resume or start fresh, cleans up stale sessions.
  </done>
</task>

<task type="auto">
  <name>Task 3: Enhance PersistedSessionPicker with detailed row UI</name>
  <files>
    Dispatch/Views/Terminal/MultiSessionTerminalView.swift
  </files>
  <action>
Replace the basic PersistedSessionPicker with full-featured UI:

1. Replace the stub PersistedSessionPicker with detailed implementation:
   ```swift
   private struct PersistedSessionPicker: View {
       let sessions: [TerminalSession]
       let onResume: (TerminalSession) -> Void
...
2. Add the PersistedSessionRow component:
   ```swift
  ...
  <verify>
Build succeeds and UI looks correct: `xcodebuild -scheme Dispatch -destination 'platform=macOS' build 2>&1 | grep -E "(error:|BUILD SUCCEEDED)"`
  </verify>
  <done>
PersistedSessionPicker has full UI with header, scrollable session list, PersistedSessionRow showing name/resumable status/relative time, and Start Fresh button.
  </done>
</task>

<task type="auto">
  <name>Task 4: Add stale Claude session detection and graceful fallback</name>
  <files>
    Dispatch/Views/Terminal/EmbeddedTerminalView.swift
    Dispatch/Services/TerminalSessionManager.swift
  </files>
...
Build succeeds and app runs: `xcodebuild -scheme Dispatch -destination 'platform=macOS' build 2>&1 | grep -E "(error:|BUILD SUCCEEDED)"`
  ...
  <done>
Stale Claude sessions are detected via terminal output patterns. Session activity is updated on prompt dispatch. Stale sessions have claudeSessionId cleared so user can close tab and reopen for fresh start.
  </done>
</task>
...
- Stale Claude sessions detected and claudeSessionId cleared
