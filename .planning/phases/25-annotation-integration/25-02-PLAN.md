...
IMPORTANT API COMPATIBILITY: Before implementing, verify these methods exist:

1. `AnnotationViewModel.copyToClipboard() async -> Bool`
   - Located at SimulatorViewModel.swift:514-516
   - Signature: `func copyToClipboard() async -> Bool`
   - Calls: `await AnnotationRenderer.shared.copyToClipboard(sendQueue)`

2. `EmbeddedTerminalService.dispatchPrompt(_:to:)`
   - Check signature in EmbeddedTerminalService.swift before using
...
4. Add canDispatch computed property:
```swift
...
6. Implement dispatch method using verified API:
```swift
private func dispatch() async {
    ...
    // Note: Verify EmbeddedTerminalService.dispatchPrompt signature matches
...
2. Grep confirms copyToClipboard call: `grep -n "copyToClipboard" Dispatch/Views/QuickCapture/QuickCaptureAnnotationView.swift`
3. Grep confirms dispatchPrompt call: `grep -n "dispatchPrompt" Dispatch/Views/QuickCapture/QuickCaptureAnnotationView.swift`
4. Launch app, open a terminal session
5. Capture screenshot (Cmd+Shift+6)
6. Session picker shows available sessions
7. Active session is pre-selected
8. Can change session selection
9. Add annotation, write prompt
10. Cmd+Enter dispatches to selected session
11. Images are in clipboard, prompt appears in terminal
  </verify>
  <done>
SessionPickerView integrated into QuickCaptureAnnotationView.
Active session auto-selected as default.
Dispatch calls verified copyToClipboard() and dispatchPrompt() APIs.
